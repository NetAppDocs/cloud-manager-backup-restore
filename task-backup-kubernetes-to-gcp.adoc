...
* You have discovered the Kubernetes cluster as a Cloud Manager working environment. link:task-kubernetes-discover-gke.html[See how to discover the Kubernetes cluster].
...
You can also link:task-restore-backups.html#restoring-volumes-from-a-kubernetes-backup-file[restore entire volumes from a backup file^] as a new volume on the same or different Kubernetes cluster in GCP (in the same region).
// or individual files
