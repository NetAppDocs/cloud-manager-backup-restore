...
=== Cloud Restore instance is no longer necessary for ONTAP Browse & Restore functionality
...
* Support for Browse & Restore operations from Google Cloud and StorageGRID storage
* Support for Browse & Restore operations when the Connector is installed on your premises

Note that the Cloud Restore instance will be removed automatically if you were previously using it.

=== Cloud Backup APIs are now available for managing backup and restore operations

The APIs are available at https://api.cloudbackup.netapp.com/docs. link:api-backup-restore.html[Go here for an overview of the APIs].
