...
* You have discovered the Kubernetes cluster as a Cloud Manager working environment. https://docs.netapp.com/us-en/cloud-manager-kubernetes/task-kubernetes-discover-aws.html[See how to discover the Kubernetes cluster^].
...
You can also link:task-restore-backups.html#restoring-volumes-from-a-kubernetes-backup-file[restore entire volumes from a backup file^] as a new volume on the same or different Kubernetes cluster in AWS (in the same region).
// or individual files
