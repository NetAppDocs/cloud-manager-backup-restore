...
* link:task-installing-linux.html[Installing the Connector on a Linux host with internet access]
...
You can also link:task-restore-backups.html[restore entire volumes from a backup file^] to a new volume on an on-premises ONTAP system.
