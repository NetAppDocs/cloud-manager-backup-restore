...
| Volume restore works with backup files stored in Amazon S3, Azure Blob, Google Cloud, and NetApp StorageGRID. File restore works with backup files stored in Amazon S3 and Azure Blob | Volume and file restore works with backup files stored in Amazon S3
...
| Browse for results across public or private clouds | Browse for results across public clouds and local Snapshots copies, and in the future, across private clouds
| Separate Cloud Restore instance is required for file restore | No Cloud Restore instance required
| No additional Amazon resources required | New bucket and Amazon Athena and AWS Glue resources required per account
| Cost associated with Cloud Restore instance when restoring files | Cost associated with Amazon Athena and AWS Glue resources when scanning your backup files and volumes for search results
...
Additional cloud providers will be supported in future releases.
