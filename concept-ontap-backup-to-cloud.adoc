...
+
** In Google, the https://cloud.google.com/bigquery[Google Cloud BigQuery service^] resources are deployed in a new bucket.
...
Cloud Backup is available in three licensing options: Pay As You Go (PAYGO) subscription, an annual contract from the AWS Marketplace, and Bring Your Own License (BYOL). A 30-day free trial is available when you get a PAYGO subscription.
