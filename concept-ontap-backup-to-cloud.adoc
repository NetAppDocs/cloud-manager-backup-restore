...
* For Volume or File Restore using Search & Restore, the Amazon Athena and AWS Glue resources are required. A cost is associated with these resources when restoring volumes and files. Search & Restore is supported only when restoring data from a backup file in Amazon S3 at this time.
