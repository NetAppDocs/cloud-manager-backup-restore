...
* If you need to restore volume data from a backup file that has been moved to archival storage (supported with AWS when using ONTAP 9.10.1+), then there's an additional per-GiB retrieval fee and per-request fee from the cloud provider.
//and Azure
...
Cloud Backup is available in a few licensing options: Pay As You Go (PAYGO) subscription, ifdef::aws[] an annual contract from the AWS Marketplace, endif::aws[] and Bring Your Own License (BYOL). A 30-day free trial is available when you initially sign up with a PAYGO subscription.

Cloud Backup is available in a few licensing options: Pay As You Go (PAYGO) subscription,
ifdef::aws[]
an annual contract from the AWS Marketplace,
endif::aws[]
and Bring Your Own License (BYOL). A 30-day free trial is available when you initially sign up with a PAYGO subscription.
