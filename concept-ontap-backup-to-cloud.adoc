...
* For Volume or File Restore using Search & Restore, the and Amazon Athena and AWS Glue resources required. A cost is associated with the Amazon Athena and AWS Glue resources when restoring volumes and files. Search & Restore is supported using AWS only at this time.
...
You'll receive a serial number that you enter in the Cloud Manager Digital Wallet page to enable the service. When either limit is reached, you'll need to renew the license. The Backup BYOL license applies to all Cloud Volumes ONTAP and on-premises ONTAP systems associated with your link:concept-cloud-central-accounts.html[Cloud Manager account^].
...
Note that you can link:task-managing-backups.html#creating-a-manual-volume-backup-at-any-time[create an on-demand backup of a volume] from the Backup Dashboard at any time, in addition to those backup files created from the scheduled backups.
...
Backup copies are associated with the https://docs.netapp.com/us-en/cloud-manager-get-started/concept-cloud-central-accounts.html[NetApp account^] in which the Connector resides.
