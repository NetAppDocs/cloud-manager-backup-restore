...
* For Volume or File Restore using Search & Restore, the Amazon Athena and AWS Glue resources required. A cost is associated with the Amazon Athena and AWS Glue resources when restoring volumes and files. Search & Restore is supported using AWS only at this time.
