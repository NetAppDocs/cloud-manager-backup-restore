...
+
If you are using port 80 for communication to the private endpoint, you are all set. You can enable Cloud Backup on the cluster now.
+
If you are using port 443 for communication to the private endpoint, you must copy the certificate from the VPC S3 endpoint and add the certificate to your ONTAP cluster, as shown in the next 4 steps.
+
