...
summary: A 30-day free trial of Cloud Backup starts when you enable the Cloud Backup service. When the free trial ends, you’ll need to pay for Cloud Backup using a pay-as-you-go (PAYGO) subscription through your cloud provider, or by purchasing a bring-your-own license (BYOL) from NetApp.
---
...
There are two annual contracts available from the AWS Marketplace for Cloud Volumes ONTAP and on-premises ONTAP systems:
...
+
If you want to use this option, set up your subscription from the Marketplace page and then https://docs.netapp.com/us-en/cloud-manager-setup-admin/task-adding-aws-accounts.html[associate the subscription with your AWS credentials^]. Note that you’ll also need to pay for your Cloud Volumes ONTAP systems using this annual contract subscription since you can assign only one active subscription to your AWS credentials in Cloud Manager.
